...
# ====================== NOVA VENDA ======================
if aba == "Nova Venda":
    # ... (seu código de nova venda continua igual - não alterei)
    ...
    with col2:
        ...
        forma = st.selectbox("Pagamento", ["Pix", "Cartão", "Boleto", "Dinheiro", "Outro"], 
                            index=0 if edit_data is None else ["Pix","Cartão","Boleto","Dinheiro","Outro"].index(edit_data['Forma_Pagamento']) if edit_data['Forma_Pagamento'] in ["Pix","Cartão","Boleto","Dinheiro","Outro"] else 0)
...
# ====================== GESTÃO DE CAIXA ======================
    ...
    obs_fechamento = st.text_area("Observação do Fechamento de Caixa")
    ...
    **Saldo Final:** R$ {saldo_inicial + entradas_vendas + entradas_extras - saidas:,.2f}
    ...
    # Histórico de Caixas
    st.subheader("📋 Histórico de Fechamentos de Caixa")
